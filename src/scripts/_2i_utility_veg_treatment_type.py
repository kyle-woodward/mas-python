...
def UtilityVegTreat(Input_Table):  # 2i Utilitiy Veg Treatment Type
    ...
    Updated_Input_Table = arcpy.management.CalculateField(in_table=Input_Table, field="UtilityID", expression="ifelse(!UtilityID!)", code_block="""def ifelse(ID):
    if ID == \"SDG&E\":
        return \"SDGE\"
    else:
        return ID""")[0]
   ...
   # Process: Calculate Treatment (Calculate Field) (management)
    Output_Table = arcpy.management.CalculateField(in_table=Intermediate_Table, field="VegetationTreatmentType", expression="ifelse(!VegetationTreatmentType!)", code_block="""def ifelse(TY):
#    if TY.find(\"Pole%\"):
    if 'tandard' in TY:
        ...
        return TY""")[0], field_type="TEXT", enforce_domains="NO_ENFORCE_DOMAINS")
