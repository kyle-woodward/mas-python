...
def TransformTreatments(treatment_poly, 
                        Input_Features,
                        CNRA_Treatments,
                        Output_Duplicate_Treatments=os.path.join(scratch_workspace, fr'Duplicate_Treatments_{Value2}')):  # 9b Transform Treatments
    ...
    CNRA_Treatments_Select = arcpy.analysis.Select(in_features=CNRA_Treatments.__str__().format(**locals(),**globals()), 
    ...
    if Value:
        Transform_Treatments_Dissolve = arcpy.management.AddField(in_table=treatment_poly, 
                                                                  field_name="GlobalID", 
                                                                  field_type="GUID", 
    ...
    if Transform_Treatments_Dissolve and Value:
        Updated_Datasets_3_ = arcpy.management.AddGlobalIDs(in_datasets=[treatment_poly])
...
#   buffer inputs
    buffered_treat2 = arcpy.analysis.Buffer(Treat_n_harves_Treatments_SpatialJoin_2_, treat_buffered2, 10)
    buffered_projects2 = arcpy.analysis.Buffer(CNRA_Treatments_Select, project_buffered2, 10)


   # Process: Append (Append) (management)
    if Transform_Treatments_Dissolve and Updated_Datasets_3_ and Value:
        treatment_poly = arcpy.management.Append(inputs=buffered_projects2, 
    ...
    if Transform_Treatments_Dissolve and Updated_Datasets_3_ and Value:
        TransformCheck(Input_Table=treatment_poly.__str__().format(**locals(),**globals()), 
