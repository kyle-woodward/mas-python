...
def WFRTFtemplate():  # 1a Template
    ...
    b_Reference_original_path = os.path.join(
        original_gdb, "b_Reference"
    )  # original b_Reference dataset in orig gdb
    ...
    if not os.path.exists(b_Reference_new_path):
        arcpy.management.CreateFeatureDataset(
            out_dataset_path=workspace, out_name="b_Reference"
        )  # {spatial_reference} is third opt arg, think don't need since we are always running with EnvManager..?

    WFRTF_Template = arcpy.management.CreateFeatureclass(
        out_path=b_Reference_new_path,
