...
def Year(Year_Input):  # 2h Calculate Year
    ...
    year_calculated = arcpy.management.CalculateField(
        in_table=Year_Input, field="Year", expression="Year($feature.ACTIVITY_END)", 
        expression_type="ARCADE", code_block="", field_type="LONG", enforce_domains="NO_ENFORCE_DOMAINS")

    # Process: Calculate Year as Text (Calculate Field) (management)
    year_txt_calculated = arcpy.management.CalculateField(
        in_table=year_calculated, field="Year_txt", expression="!Year!", 
    ...
    year_fed_fy = arcpy.management.CalculateField(
        in_table=year_txt_calculated, field="Federal_FY", expression="ifelse(!ACTIVITY_END!)", 
        expression_type="PYTHON3", code_block="""def ifelse(DATE):
    ...
    year_final = arcpy.management.CalculateField(
        in_table=year_fed_fy, field="State_FY", expression="ifelse(!ACTIVITY_END!)", 
        expression_type="PYTHON3", code_block="""def ifelse(DATE):
