...
def TransformProjects(Input_Features,
                      ...
                      Output_Duplicate_Projects=os.path.join(scratch_workspace, fr'Duplicate_Projects_{Value2}')):  # 9a Transform Projects
    ...
    CNRA_Projects_Select = arcpy.analysis.Select(in_features=CNRA_Projects.__str__().format(**locals(),**globals()), 
    ...
    if Value:
        Treat_n_harves_Project_SpatialJoin1_2_ = arcpy.management.AddField(in_table=project_poly, 
                                                                           field_name="GlobalID", 
                                                                           field_type="GUID", 
    ...
    if Treat_n_harves_Project_SpatialJoin1_2_ and Updated_Datasets_4_ and Value:
        treat_spatialjoin = arcpy.management.CalculateField(in_table=Treat_n_harves_Project_SpatialJoin1_2_, 
                                                                                 field="GlobalID", 
...
#   buffer inputs
    ...
    buffered_projects = arcpy.analysis.Buffer(CNRA_Projects_Select, project_buffered, 10)

# Process: Append (2) (Append) (management)
    if Treat_n_harves_Project_SpatialJoin1_2_ and Updated_Datasets_4_ and Value:
        project_poly = arcpy.management.Append(inputs=buffered_projects, 
    ...
    if Treat_n_harves_Project_SpatialJoin1_2_ and Updated_Datasets_4_ and Value:
        TransformCheck(Input_Table=project_poly.__str__().format(**locals(),**globals()), 
